...
def class_model(df):
	...
	log_cross_val = cross_val_score(model, x, y, cv=10, scoring='accuracy')
	print('Classification Results with cross validation::')
	log_cv_accuracy = log_cross_val.mean()
	print("Accuracy: ",log_cv_accuracy)

	log_cross_val_pre = cross_val_score(model, x, y, cv=10, scoring='precision_macro')
	log_cv_precision = log_cross_val_pre.mean()
	print("Precision: ",log_cv_precision)

	log_cross_val_re = cross_val_score(model, x, y, cv=10, scoring='recall_macro')
	log_cv_recall = log_cross_val_re.mean()
	print("Recall: ",log_cv_recall)

	log_cross_val_f1 = cross_val_score(model, x, y, cv=10, scoring='f1_macro')
